...
export const Bitacoras = () => {
 ...
 /**
  ...
  const CustomDateInput = ({ value, onClick }) => (
    <button type="button" className="btn btn-outline-secondary btn-sm d-flex align-items-center" onClick={onClick}>
      <FaCalendarAlt className='mr-2' size={16} />
      <span className="border rounded p-1">{value || 'Seleccionar fecha...'}</span>
    </button>
  );
          ...
          <form onSubmit={(e) => handleBitacora(e, selectedBitacora.id_bitacora)} className='flex flex-col gap-1.5'>
              ...
              <div className='flex flex-row items-center gap-2'>
                ...
                <div className='flex flex-col '>
                  ...
                  <div className='flex justify-end mb-4'>
                    <label htmlFor='bitacora-select' className='mr-2'>Seleccionar número de bitácora: </label>
                  ...
                  <span className={`text-xs  ${selectedBitacora.calificacion_bitacora === 'Calificado' ? 'visible' : 'hidden'}`}>
                    {selectedBitacora.fecha_modificacion}
                  </span>
                </div>
              </div>
            ...
            <section className='flex flex-col gap-1 px-2 text-sm'>
              ...
              <div className='flex flex-col gap-1'>
                <label className='text-sm' htmlFor='observaciones_fecha'>Fecha de entrega de bitácora:
                <DatePicker
                  selected={isValidDate(new Date(selectedBitacora.observaciones_fecha)) ? new Date(selectedBitacora.observaciones_fecha) : null}
                  onChange={handleDateChange}
                  name='observaciones_fecha'
                  dateFormat='yyyy/MM/dd'
                  className='w-full h-10 p-2 text-sm border-gray-300 focus:outline-none rounded-xl'
