      ...
      <section className="grid grid-rows-3-10-75-15 flex-auto w-min relative">
        ...
        <div className="grid grid-cols-1 gap-1 p-4 sm:grid-cols-2 md:grid-cols-3">
          {apprentices.map((apprentice, i) => {
            return <Card cardUser shadow={'shadow-2xl'} scale={'scale-90'} title={`${apprentice.nombres_inscripcion} ${apprentice.apellidos_inscripcion}`} subtitle={apprentice.correo_electronico_inscripcion} lione={apprentice.programa_formacion_aprendiz_inscripcion} litwo={apprentice.numero_ficha_aprendiz_inscripcion} key={i} roundedLink={'rounded-xl'} borderColor={'border-primary'} buttonText={'Más información'} link={'/home'} />
          })}
        </div>
        <Footer />
      </section>
