...
const Card = ({ title, titleColor, description, buttonText, bgColor, link, scale, img = 'https://unavatar.io/iLestar', subtitle, shadow, lione, litwo, cardUser = false, cardHome = false, alt = 'foto user', marginLink, borderColor, roundedLink }) => {
  return (
    <div className={`${bgColor} bg-opacity-50 ${shadow} rounded-lg p-3 flex flex-col justify-center h-auto ${scale && 'scale-90'}`}>
      ...
      {link && (
        <Link to={link} className={`${roundedLink} border-1 ${borderColor} p-1 w-fit justify-self-end ${marginLink} mt-5 font-semibold text-xs`}>
          {buttonText}
        </Link>
      )}
    </div>
