...
export const createPracticalStage: RequestHandler<{}, Response, PracticalStages> = async ({ body }: Request<{}>, res: Response): Promise<Response> => {
  const { tipo_modalidad_practica } = body
  try {
    await connection.query('INSERT INTO modalidades_etapa_practica (tipo_modalidad_practica) VALUE (?)', [tipo_modalidad_practica])
...
export const editPracticalStage: RequestHandler<{}, Response, PracticalStages> = async (req: Request, res: Response): Promise<Response> => {
  ...
  const idNumber = Number(id)
  try {
    const [practicalStage] = await connection.query('UPDATE modalidades_etapa_practica SET tipo_modalidad_practica = IFNULL(?, tipo_modalidad_practica) WHERE id_modalidad_practica = ?', [tipo_modalidad_practica, idNumber])
